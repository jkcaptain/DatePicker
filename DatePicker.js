;(function(){
	...
	//获取translate属性值
	//code refer https://github.com/ElemeFE/mint-ui/blob/master/packages/picker/src/translate.js line 34
	...
	//设置translate属性值
	//code refer https://github.com/ElemeFE/mint-ui/blob/master/packages/picker/src/translate.js line 48
	function translateElem(elem, x, y){
		...
		elem.style[TRANSFORM_KEY] = 'translate(' + x +'px,' + y + 'px) translateZ(0px)';
	}
	//清空translate属性值
	//code refer https://github.com/ElemeFE/mint-ui/blob/master/packages/picker/src/translate.js line 75
	...
	*/
	function DatePicker(container, options){
		...
		//输入的年		
		var inpYearArr = options.year && options.year.split('-');
		if(!!inpYearArr){
		...
		function _createDays(year, month){
			...
			//获取当前年月，总共有多少天
			var daysLen = new Date(parseInt(year), parseInt(month), 0).getDate();	
		...
		var viewLength = Math.floor(container.parentNode.offsetHeight / itemHeight);     //容器能放几个item
		var yearListHeight = _getContentHeight(itemHeight, containerChild[0].children.length);
		var monthListHeight = _getContentHeight(itemHeight, containerChild[1].children.length);
		var daysListHeight = _getContentHeight(itemHeight, containerChild[2].children.length);
		//获取list的高度
		...
		function _getDragRange(elem){
			...
			if(elem && elem instanceof HTMLElement){
				min = -itemHeight * (elem.children.length - Math.ceil(viewLength/2));
			}
			return {
				min: min, 
		...
		function _getTransByValue(elem, value){
			...
			//console.log(_getDragRange().max - curIndex * itemHeight)
		...
		var _dragEvents = {
			...
			move: function(e){
				...
				//code refer https://github.com/ElemeFE/mint-ui/blob/master/packages/picker/src/
				...
				translate = Math.max(Math.min(translate, dranRange.max), dranRange.min);
				...
				//如果是月份, 就触发selected, 更新天数
				//curIndex === 0代表变化的是年份， curIndex === 1代表变化的是月份
				if (curIndex === 0 || curIndex === 1) {
					var daysListElement;	
					if (curIndex === 0){
						daysListElement = elem.nextElementSibling.nextElementSibling;
					} else {
						daysListElement = elem.nextElementSibling;
					}				
					_insertNewDays(daysListElement, selectedDate[0], selectedDate[1]);
					...
					var newDaysListState = _getValueByTran(daysListElement, curDayListTop);
					selectedDate[2] = newDaysListState.value;
					daysListElement.children[newDaysListState.index].classList.add('selected');
				}					

				//更新selectedTime
				selectedTime = selectedDate.join(separator);
				console.log(curIndex, selectedDate, selectedTime);								

				//触发回调
				(typeof selectCallback === 'function') && selectCallback.call(null);
		...
		function translateTo(value){	
			...
			valueToArr.forEach(function(value, index){
				...
				elem.children[selectedIndexArr[index]].classList.remove('selected');
